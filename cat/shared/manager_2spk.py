...
# Apache 2.0.
# Author: Xiangzhu Kong, Huahuan Zheng (maxwellzh@outlook.com)

"""training/evaluating manager
   用于非simu任务, 输出chunk-loss,utt-loss
"""
...
class Manager(object):
    def __init__(
        ...
        # # Multiply the learning rate by 10 for simu parameters
        ...
        if args.bf is not None:
            coreutils.distprint(f"> initialize bf model from: {args.bf}", args.gpu)
            checkpoint = torch.load(
                args.bf, map_location=f"cuda:{args.gpu}"
            )  # type: OrderedDict

            try:
                # self.model.load_state_dict(checkpoint["model"])
                model_dict = self.model.state_dict()
                # 提取预训练模型中的beamformer参数
                pretrained_beamformer_dict = {
                    k: v for k, v in checkpoint['model'].items() if k.startswith('module.beamformer.beamformer')
                }
                
                # 更新当前模型的状态字典
                model_dict.update(pretrained_beamformer_dict)
        ...
        if args.am is not None:
            coreutils.distprint(f"> initialize am model from: {args.am}", args.gpu)
            checkpoint = torch.load(
                args.am, map_location=f"cuda:{args.gpu}"
            )  # type: OrderedDict
            ...
            try:
                ...
                # 提取预训练模型中的beamformer参数
                pretrained_beamformer_dict = {
                    k: v for k, v in checkpoint['model'].items() if k.startswith('module.encoder')
                }
        ...
        if args.unfreeze is not None:
            ...
            # 假设 args.freeze 是一个字符串，包含要解冻的模块名称
            modules_to_unfreeze = args.unfreeze.split(',')
...
def train(
    ...
    def _go_step(minibatch) -> Tuple[torch.Tensor, int]:
        ...
        with autocast(enabled=use_amp):
            if hook_func is None:
                loss, utt_loss, chunk_loss = model(*minibatch)
            else:
            ...
            utt_loss = raw_loss.new_tensor(utt_loss)
            utt_loss = utt_loss.detach().clone()
            
            chunk_loss = raw_loss.new_tensor(chunk_loss)
            chunk_loss = chunk_loss.detach().clone()
        ...
        #         raise ValueError(f'Grad has nan or inf for parameter: {name}')
        ...
        # return for logging
        return raw_loss, feats.size(0),utt_loss, chunk_loss
    ...
    for i, minibatch in enumerate(trainloader):
        #fold_count += minibatch[1].size().numel()
        ...
        if cnt_step_update + manager.step_by_last_epoch < manager.step:
            ...
            #if fold == 1 or (fold_count + 1) % fold == 0:    
                cnt_step_update += 1
                p_bar.update()
        ...
        if fold == 1 or (i + 1) % fold == 0:
        #if fold == 1 or (fold_count + 1) % fold == 0:  
        # 或者可以改为，打开梯度累积操作 flag，统计n_batch，当其 >= batch_size的时候自动进行一次梯度更新
            local_loss, local_bs, utt_loss, chunk_loss = _go_step(minibatch)
            accum_loss += local_loss * local_bs
            accum_utt_loss += utt_loss * local_bs
            accum_chunk_loss += chunk_loss * local_bs
            n_batch += local_bs
            ...
            #     for name, param in model.named_parameters():
            #         if param.grad is not None:
            #             manager.writer.add_scalar(f'Gradient/{name}', param.grad.norm(), global_step=manager.step)    
            manager.step += 1
            ...
            item_utt_loss = (accum_utt_loss / n_batch).item()
            item_chunk_loss = (accum_chunk_loss / n_batch).item()

            # update tensorboard
            if args.rank == 0:
                manager.writer.add_scalar("loss_train/total", item_loss, manager.step)
                manager.writer.add_scalar("lr", scheduler.lr_cur, manager.step)
                manager.writer.add_scalar("loss_train/utt_loss", item_utt_loss, manager.step)
                manager.writer.add_scalar("loss_train/chunk_loss", item_chunk_loss, manager.step)
...
def evaluate(
    ...
    for minibatch in tqdm(
        ...
        total_utt_loss += utt_loss.float() * feats.size(0)
        total_chunk_loss += chunk_loss.float() * feats.size(0)
    ...
    dist.all_reduce(total_utt_loss, dist.ReduceOp.SUM)
    dist.all_reduce(total_chunk_loss, dist.ReduceOp.SUM)
    
    dist.all_reduce(cnt_seq, dist.ReduceOp.SUM)
    
    avg_loss = (total_loss / cnt_seq).item()
    avg_utt_loss = (total_utt_loss / cnt_seq).item()
    avg_chunk_loss = (total_chunk_loss / cnt_seq).item()

    if args.rank == 0:
        manager.writer.add_scalar("loss_dev/total_loss", avg_loss, manager.step)
        manager.writer.add_scalar("loss_dev/utt_loss", avg_utt_loss, manager.step)
        manager.writer.add_scalar("loss_dev/chunk_loss", avg_chunk_loss, manager.step)
    return avg_loss
