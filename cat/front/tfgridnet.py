...
class TFGridNet(nn.Module):
    ...
    def forward(
        ...
        # (B, T, C, F, 2)
        #n_batch, n_frames, _, n_freqs, _ = batch.shape
        
        batch0 = input.transpose(1, 2)  # [B, M, T, F]
        
        batch = torch.cat((batch0[..., 0], batch0[..., 1]), dim=1)  # [B, 2*M, T, F]
...
if __name__ == "__main__":
    ...
    # (B, T, C, F, 2)
    input_data = torch.randn(Batch, Time, Channels, Frequency, 2)
    ilens = torch.ones(1,Batch)*Time
    
    model = TFGridNet()
    output, _, _ = model(input_data, ilens)
    # 打印输出结果的形状
    ...
    model_sz = model_size(model)

    print(f'Number of parameters: {num_params}')
    print(f'Model size (MB): {model_sz:.2f}')