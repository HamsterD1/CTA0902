...
# Apache 2.0.
# Acknowlegement: 
#   This code is adapted from the SH_injection project. 
#   The original code can be found at https://github.com/Pandade1997/SH_injection.git
...
class IGCRN(nn.Module):
    def __init__(self, in_ch_sph=25, in_ch_stft=1, out_ch=1, channels=32, n_fft=512, hop_length=256):
        super().__init__()
        self.n_fft = n_fft
        ...
        self.in_ch_sph = in_ch_sph
        self.in_ch_stft = in_ch_stft
    ...
    def forward(self, input, ilen=None):
        # x: b m t
        ...
        sph_input = input[:, :self.in_ch_sph,...]
        mix_input = input[:, -self.in_ch_stft:, ...]
        
        # (b, m, F, T, 2)
        sph_single = torch.cat([sph_input[...,0], sph_input[...,1]], dim=1)
        mix_single = torch.cat([mix_input[...,0], mix_input[...,1]], dim=1)
...
if __name__ == '__main__':
    ...
    input_2 = Variable(torch.FloatTensor(torch.rand(1, 16, 257, 63)))
    net = IGCRN(in_ch_sph=25, in_ch_stft=8)
    output = net(input_1, input_2)
