...
# Apache 2.0.
# Acknowlegement: 
#   This code is adapted from the EaBNet project. 
#   The original code can be found at https://github.com/Andong-Li-speech/EaBNet.git
...
class EaBNet(nn.Module):
    ...
    def forward(self, inpt: Tensor, ilens):
        """
        :param inpt: (B, T, F, M, 2) -> (batchsize, seqlen, freqsize, mics, 2)
        :return: beamformed estimation: (B, 2, T, F)
        """
        
        # inpt = inpt[:,:,[0,1,2,3,4,5,6,7],...]
        # # 指定要保留的通道索引
        # keep_channels = torch.tensor([1,5])
        # # 构建掩码，形状为 (C, )
        # mask = torch.zeros(8, dtype=torch.bool)
        # mask[keep_channels] = True
        # # 扩展掩码以匹配 inputs 的维度
        # mask = mask.unsqueeze(0).unsqueeze(1).unsqueeze(-1).unsqueeze(-1)  # (1, 1, C, 1, 1)
        # mask = mask.to(inpt.device)  # 将 mask 移动到相同的设备
        # # 将未选择的通道置为 0
        # inpt = inpt * mask
        
        
        # (B, T, C, F, 2) --> (B, T, F, C, 2)
        inpt = inpt.permute(0,1,3,2,4)
        ...
        if self.topo_type == "mimo":
            ...
            output = torch.stack((esti_x_r, esti_x_i), dim=1).permute(0,2,3,1)
            return output, ilens, bf_w
