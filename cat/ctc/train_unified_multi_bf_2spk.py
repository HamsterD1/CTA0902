...
from .train_raw_multi_bf_2spk import AMTrainer, build_model as am_builder, main_worker as basic_worker
...
class UnifiedAMTrainer(AMTrainer):
    ...
    def bf_chunk_infer(
        ...
        stream_chunk, _ = self.trans.stft_to_fbank(inputs_with_context,flens1)
    ...
    def forward(self, audio, lx, labels, ly):
        ...
        samples_utt = []
        # 假设 inputs_with_context 是一个包含 self.num_spk 个数量的 tuple
        for spk in range(self.num_spk):
            
            inputs_with_context_temp = inputs_with_context[spk]
            if torch.isnan(inputs_with_context_temp).any():
                print("feats array contains NaN values!")
                raise TypeError

            # utt cat 
            samples = inputs_with_context_temp[:, left_context_size:chunk_size + left_context_size, :]
            samples = samples.contiguous().view(samples.size(0)//num_chunks, 
                                                samples.size(1)*num_chunks, 
                                                samples.size(2), 
                                                samples.size(3))
            
            samples_utt.append(samples)
        
        # torch.Tensor(self.num_spk*Batch, Frames, Freq)
        inputs_with_context = torch.cat(inputs_with_context, dim=0)
        samples = torch.cat(samples_utt, dim=0)
        
        
        # utt feature exaction
        samples, _ = self.trans.stft_to_fbank(samples,flens)
        ...
        if self.simu:
            ...
            if float('inf') == loss + chunk_loss :
                print("loss + chunk_loss  is inf!")
                assert False
        
            if math.isnan(loss + chunk_loss ):
                print("loss is NaN!")  
                assert False
            return loss + chunk_loss, loss, chunk_loss
